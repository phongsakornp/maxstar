...
class App:
    def __init__(self, stdscr):
        ...
        # Node-side telemetry (the spoken "connected"/"disconnected"
        # announcement app_rpt plays for every link event on this node,
        # heard by everyone linked -- not something maxstar generates).
        # Assumes on at startup, matching this node's telemdefault=2;
        # cop,34/cop,35 (see toggle_telemetry) were added to rpt.conf's
        # functions table specifically to make this toggleable.
    ...
    def toggle_telemetry(self):
        """Toggle app_rpt's node-side telemetry (the spoken connect/
        disconnect announcement) -- a real node-wide setting, not
        something maxstar can suppress client-side, since it's
        generated by app_rpt for everyone linked, not sent as separate
        per-listener audio. Uses cop,35 to turn on, not cop,33: cop,33
        sets telemmode to a special always-on sentinel (0x7fffffff)
        that isn't the same as this node's actual configured default
        and didn't produce the connect/disconnect announcement, while
        cop,35 sets telemmode=1 -- exactly what telemdefault=2 sets at
        startup, confirmed against app_rpt's own source
        (rpt_config.c/rpt_functions.c). cop,34 (off) sets telemmode=0
        either way."""
        call = self.call
        if not call or self.telemetry_busy:
            return
        turning_on = not self.telemetry_on
        digits = "*935" if turning_on else "*934"
        ...
        def worker():
            try:
                send_dtmf_function(call, digits)
                self.telemetry_on = turning_on
                self.link_status = "telemetry on" if turning_on \
                    else "telemetry off"
            finally:
                self.telemetry_busy = False
