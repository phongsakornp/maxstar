...
class App:
    def __init__(self, stdscr):
        ...
        # cop,33/cop,34 were added to rpt.conf's functions table
        # specifically to make this toggleable.
    ...
    def toggle_telemetry(self):
        ...
        disconnect announcement) via cop,33/cop,34 -- a real node-wide
        setting, not something maxstar can suppress client-side, since
        it's generated by app_rpt for everyone linked, not sent as
        separate per-listener audio."""
        call = self.call
        if not call or self.telemetry_busy:
            return
        turning_on = not self.telemetry_on
        digits = "*933" if turning_on else "*934"
    ...
    def draw_monitor(self):
        ...
        # A_DIM alone isn't reliably rendered by every terminal, so lean
        # on the same lowercase-vs-uppercase + bold contrast the RX/TX
        # badges already use for active/inactive, not just the attribute.
        if self.telemetry_on:
            telem_attr = curses.color_pair(CP_GREEN) | curses.A_BOLD
            telem_text = "TELEM ON  "
        else:
            telem_attr = curses.color_pair(CP_DIM) | curses.A_DIM
